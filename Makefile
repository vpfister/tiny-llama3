...
test: hidden ## run pytest with coverage
	poetry run pytest -v --cov tiny_llama3
...
pyinstaller: install lint test ## Create a binary executable using pyinstaller
	poetry run pyinstaller src/tiny_llama3/cli.py --onefile --name tiny-llama3
jupyter: ## run the jupyter-lab server
	poetry run jupyter-lab
run: ## run `poetry run tiny-llama3`
	poetry run tiny-llama3
